...
const getTopLevelDependencies = _.memoize((specifier) => {
  ...
  return _.chain(json)
    .thru(JSON.parse)
    .thru((data) => {
      return _.isPlainObject(data) ? _.toPairs(data) : _.flatMap(data, _.toPairs);
    })
    .map(([ name, version ]) => {
      return `${name}@${version}`;
    })
...
/**
 * @summary Recursively fetch the dependency tree of an npm specified
 * @function
 * @private
 *
 * @param {String} specifier - npm specifier
 ...
 *
 * @example
 * const dependencies = getDependencyTree('drivelist@^5.0.0');
 */
const getDependencyTree = (specifier) => {
  const topLevelDependencies = getTopLevelDependencies(specifier);
  const nestedDependencies = _.flatMapDeep(topLevelDependencies, getDependencyTree);
...
const blacklist = _.uniq(normalizeNpmQualifiers(
  ...
  _.flatMap(platformSpecificDependencies, getDependencyTree))));
